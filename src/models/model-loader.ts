...
export async function modelLoad(url: string, type: ModelType) {
  return new Promise<MeshData[]>((resolve, reject) => {
    ModelLoaders[type as keyof typeof ModelLoaders].load(
      url,
      (object) => {
        const meshDatas: MeshData[] = [];
        console.log("obj loader", object);
...
export async function materialLoad(url: string, type: ModelType) {
  return new Promise<{ [key: string]: Material }>((resolve, reject) => {
    MaterialLoaders[type as keyof typeof MaterialLoaders].load(
      url,
      (material) => {
        const materials: {
          [key: string]: Material;
...
export function combineMeshGroup(
  meshData: MeshData[],
