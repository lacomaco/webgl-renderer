...
export class Model {
  ...
  async init() {
    ...
    const vertexShader = ShaderProgram.createShader(
      this.gl,
      shader.vs,
      this.gl.VERTEX_SHADER,
    );
    const fragmentShader = ShaderProgram.createShader(
      this.gl,
      shader.fs,
  ...
  worldMove() {
    ...
    glm.mat4.rotateY(
      move,
      move,
      (this.worldData.currentYRotate * Math.PI) / 180,
    );
  ...
  changeTextureBuffer(mtl: Material) {
    const u_imageLocation = this.gl?.getUniformLocation(
      this.program!,
      "u_diffuseMap",
    );
  ...
  createUniformBuffer() {
    ...
    this.uniformBuffer.ambient = this.gl.getUniformLocation(
      this.program,
      "u_materialAmbient",
    );
    this.uniformBuffer.diffuse = this.gl.getUniformLocation(
      this.program,
      "u_materialDiffuse",
    );
    this.uniformBuffer.specular = this.gl.getUniformLocation(
      this.program,
      "u_materialSpecular",
    );
    this.uniformBuffer.shininess = this.gl.getUniformLocation(
      this.program,
      "u_materialShininess",
    );
  }
  ...
  setWorldUniformBuffer() {
    const worldMatrixLocation = this.gl?.getUniformLocation(
      this.program!,
      "u_world",
    );
    this.gl?.uniformMatrix4fv(worldMatrixLocation!, false, this.worldMatrix);

    const worldInvTransposeMatrixLocation = this.gl?.getUniformLocation(
      this.program!,
      "u_worldInvTranspose",
    );
    ...
    this.gl?.uniformMatrix4fv(
      worldInvTransposeMatrixLocation!,
      false,
  ...
  render() {
    ...
    Object.keys(this.parts).forEach((materialName) => {
      ...
      for (const obj of objs) {
        ...
        this.gl.drawArrays(this.gl.TRIANGLES, currentOffset, vertexCount);
