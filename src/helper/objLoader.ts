...
export function parseMTL(text: string): Materials {
  ...
  const keywords = {
    ...
    //
    map_Ka(parts: any, unparsedArgs: any) {
      material.ambientMap = parseMapArgs(unparsedArgs);
    },
    // Diffuse Map <-텍스처
    map_Kd(parts: any, unparsedArgs: any) {
      material.diffuseMap = parseMapArgs(unparsedArgs);
    },
    // Specular Map <- 광택
    map_Ns(parts: any, unparsedArgs: any) {
      material.specularMap = parseMapArgs(unparsedArgs);
    },
    // Bump map <- 요철같은 세세한 질감
    map_Bump(parts: any, unparsedArgs: any) {
